...
import { getModule, getSectionsByModule, deleteSection } from '../../../shared/lib/api';
import { getUploadedContentByModule, UploadedContent } from '../../../shared/services/api/uploadedContent';
...
export default function ModuleSections() {
  ...
  const loadModuleAndSections = async () => {
    try {
      ...
      // Load uploaded content for this module
      try {
        const contentData = await getUploadedContentByModule(parseInt(moduleId!));
        setUploadedContent(contentData);
      } catch (contentError) {
        console.error('Error loading uploaded content:', contentError);
        setUploadedContent([]);
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-xs"></div>
  ...
  if (error || !module) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-xs">{error || 'Module not found'}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-sm mb-sm">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          {sections.length === 0 ? (
            <div className="text-center py-12">
              <FileText className="h-12 w-12 text-gray-400 mx-auto mb-xs" />
              <p className="text-gray-500 mb-xs">No sections found for this module</p>
              <Link
                to={`/modules/${moduleId}/sections/new`}
                className="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700"
              >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-medium text-gray-900">Training Materials</h2>
                <p className="text-sm text-gray-600">Content uploaded for this module</p>
              </div>
              <Link
                to="/upload-content"
                className="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700"
          ...
          {uploadedContent.length === 0 ? (
            <div className="text-center py-12">
              <UploadIcon className="h-12 w-12 text-gray-400 mx-auto mb-xs" />
              <p className="text-gray-500 mb-xs">No training materials uploaded for this module yet</p>
              <Link
                to="/upload-content"
                className="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700"
              >
                  ...
                  <div key={content.id} className="px-6 py-4 hover:bg-gray-50">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        {content.scribeLink && (
                          <a
                            href={content.scribeLink}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"
                            title="View in Scribe"
                          >
