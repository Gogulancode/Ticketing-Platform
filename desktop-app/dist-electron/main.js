...
const gotTheLock = require$$1$2.app.requestSingleInstanceLock();
if (!gotTheLock) {
...
let mainWindow = null;
let tray = null;
const isDev = process.env.NODE_ENV === "development" || !require$$1$2.app.isPackaged;
...
const getResourcePath = (...paths) => {
  if (require$$1$2.app.isPackaged) {
    return require$$0$1.join(process.resourcesPath, "app", ...paths);
  }
  return require$$0$1.join(__dirname, "..", ...paths);
};
...
function createTray() {
  ...
  let icon = require$$1$2.nativeImage.createFromPath(iconPath);
  if (!icon.isEmpty()) {
  ...
  const contextMenu = require$$1$2.Menu.buildFromTemplate([
    ...
    { type: "separator" },
    {
      ...
      click: () => {
        mainWindow == null ? void 0 : mainWindow.show();
        mainWindow == null ? void 0 : mainWindow.webContents.send("navigate", "/tickets");
      }
    },
    {
      label: "📋 My Tickets",
      click: () => {
        mainWindow == null ? void 0 : mainWindow.show();
        mainWindow == null ? void 0 : mainWindow.webContents.send("navigate", "/my-tickets");
      }
    },
    {
      label: "➕ Create Ticket",
      click: () => {
        mainWindow == null ? void 0 : mainWindow.show();
        mainWindow == null ? void 0 : mainWindow.webContents.send("navigate", "/tickets/new");
      }
    },
    { type: "separator" },
    {
...
require$$1$2.ipcMain.handle("get-store-value", (_, key) => {
  return store.get(key);
});
require$$1$2.ipcMain.handle("set-store-value", (_, key, value) => {
  store.set(key, value);
});
require$$1$2.ipcMain.handle("get-server-url", () => {
  return store.get("serverUrl");
});
require$$1$2.ipcMain.handle("set-server-url", (_, url) => {
  store.set("serverUrl", url);
});
require$$1$2.ipcMain.handle("show-notification", (_, options) => {
  if (require$$1$2.Notification.isSupported()) {
    const notification = new require$$1$2.Notification({
      title: options.title,
      body: options.body,
      icon: require$$0$1.join(__dirname, "../public/icon.svg")
    });
    notification.show();
...
require$$1$2.ipcMain.handle("set-badge-count", (_, count) => {
  var _a;
  if (!mainWindow) return;
  if (process.platform === "darwin") {
    (_a = require$$1$2.app.dock) == null ? void 0 : _a.setBadge(count > 0 ? String(count) : "");
  }
  if (tray) {
...
require$$1$2.ipcMain.handle("get-app-version", () => {
  return require$$1$2.app.getVersion();
});
require$$1$2.ipcMain.handle("get-platform", () => {
  return process.platform;
});
require$$1$2.app.on("open-url", (event, url) => {
  event.preventDefault();
  mainWindow == null ? void 0 : mainWindow.webContents.send("deep-link", url);
});
require$$1$2.app.on("web-contents-created", (_, contents) => {
